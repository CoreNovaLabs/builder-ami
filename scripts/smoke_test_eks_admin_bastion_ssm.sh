...
REMOTE_COMMANDS=(
  'set -euo pipefail'
  'corenova-eks-check'
  'aws --version 2>&1 | grep -F "aws-cli/2.36.42 "'
  'test "$(readlink -f /usr/local/bin/kubectl)" = /opt/corenova/eks/bin/kubectl-v1.36.4'
  'for version in v1.31.14 v1.32.13 v1.33.13 v1.34.11 v1.35.8 v1.36.4; do test -x "/opt/corenova/eks/bin/kubectl-${version}"; done'
  '! find /opt/corenova/eks/bin -maxdepth 1 -name "kubectl-v1.30.*" -print -quit | grep -q .'
  'helm version --short | grep -E "^v3\.22\.0(\+|$)"'
  'test "$(eksctl version)" = 0.230.0'
  'yq --version | grep -F "version v4.53.6"'
